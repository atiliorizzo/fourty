...
- **Código ya iniciado** en `packages/rules-engine` (monorepo con npm workspaces, TypeScript, tests con Vitest):
  ...
  - `src/houseRules.ts` + `src/dataState.ts`: el **orquestador de una data completa**. Sigue el patrón de *reducer* (como un aggregate de DDD que expone `Handle(command) => (newState, events)`): las acciones son `announceRonda`, `playCard`, `playCardForSuma` y `claimFalla`, cada una devuelve `{ state, events }` sin mutar nada. Internamente reparte nuevas manos de 5 cartas automáticamente cuando se agotan las manos y queda mazo (reabriendo la ventana de canto de ronda), y cierra la data otorgando las cartas sueltas de la mesa al equipo de la última captura cuando se agotan mazo y manos.
  - 62 tests unitarios pasando.
  - **Simplificaciones/asunciones tomadas al construir el orquestador (a revisar si algo no coincide con cómo se juega realmente)**:
    - "Caída a una ronda" se detecta comparando el rango capturado contra cualquier ronda anunciada por el equipo rival en esa data — no verifica estrictamente que la captura ocurra en el turno inmediatamente siguiente al canto, solo que sea el equipo rival quien la capture en algún momento de la data.
    - Si una data completa termina sin que NINGÚN equipo haya capturado nunca una carta (caso muy raro), las cartas sueltas de la mesa no tienen a quién asignarse (no hay "último equipo capturador") y quedan sin repartir — no está resuelto qué debería pasar ahí.
    - La ventana para cantar ronda/doble ronda se reabre en cada mini-reparto de 5 cartas dentro de la data (no solo en el primero), interpretando "su mano repartida" como cualquier mano nueva recibida durante la data.
- **Pendiente de definir/implementar en el motor de reglas**: el ritual de "sacar carta más alta para determinar quién reparte" (se va a resolver más adelante con un sorteo simple, no bloquea nada), la capa "Chica" que envuelve varias datas acumulando puntos hasta 40 (con "38 que no juega" y el resto de victorias automáticas: 4 caídas seguidas, +15 de cartón), y las capas de Mesa/Torneo/Juez de Aguas por encima de eso.
